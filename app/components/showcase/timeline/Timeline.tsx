...
const Timeline = ({ progress }: { progress: number }) => {
  ...
  const timeline = useMemo(() => TIMELINE_POINTS, []);
    ...
    <group position={[0, -0.1, -0.1]}>
      ...
      <group ref={groupRef}>
        {visibleTimelinePoints.map((point, i) => {
          ...
          return <TimelineNode point={point} key={i} diff={diff} />;
        })}
      </group>
    </group>
  );
