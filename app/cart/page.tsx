...
const CHECKOUT_DETAILS_KEY = "checkout-details";
...
export default function CartPage() {
	...
	useEffect(() => {
		...
		try {
			...
			setCart(normalizeCart(parsedCart));

			const rawDetails = window.localStorage.getItem(CHECKOUT_DETAILS_KEY);
			if (rawDetails) {
				const parsedDetails = JSON.parse(rawDetails) as {
					name?: string;
					email?: string;
					phone?: string;
					address?: string;
	...
	}, [name, email, phone, address, isCartHydrated]);
					...
					<>
						...
						<div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
							<section className="lg:col-span-7 space-y-4">
								...
								{!isCartHydrated ? null : cart.length === 0 ? (
									<div className="text-center py-20 border border-white/20 bg-black/20 backdrop-blur-sm rounded-[10px]">
										<p className="font-sans text-white/75 mb-8">
											Your cart is empty.
										</p>
										<Link
											href="/shop"
											className="inline-flex items-center px-8 py-3 border border-[#bfc6cc] text-[#bfc6cc] font-sans text-[10px] uppercase tracking-[0.28rem] hover:bg-[#bfc6cc]/25 transition-colors rounded-[10px]"
										>
											Shop Prints
										</Link>
									</div>
